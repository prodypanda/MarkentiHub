...
describe("Crypto — Webhook Signatures", () => {
  it("should generate and verify a valid signature", () => {
    const payload = JSON.stringify({ order_id: "pd_order_123", amount: 85.0 });
  ...
  it("should not throw on different signature length and return false", () => {
    const payload = JSON.stringify({ amount: 100 });
    const secret = "webhook_secret";
    ...
    expect(() => {
      isValid = verifyWebhookSignature(payload, signature, secret);
    }).not.toThrow();
