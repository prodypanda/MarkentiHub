...
class PdApiClient {
  ...
  async get<T = any>(path: string, params?: Record<string, string>) {
    return this.request<T>("GET", `/api${path}`, { params });
  }
  async post<T = any>(path: string, body?: any) {
    return this.request<T>("POST", `/api${path}`, { body });
  }
  async put<T = any>(path: string, body?: any) {
    return this.request<T>("PUT", `/api${path}`, { body });
  }
  ...
  async getStore(id: string) {
    return this.request("GET", `/api/pd/stores/${id}`);
  }
  async getProduct(id: string) {
    return this.request("GET", `/store/products/${id}`);
  }
  async getPlans() {
    return this.request("GET", "/api/pd/subscriptions/plans");
  }
  async getWallet() {
    return this.request("GET", "/api/pd/wallet");
  }
  async getVerificationStatus() {
    return this.request("GET", "/api/pd/verification");
  }
  async getNotifications(page = 1) {
    return this.request("GET", "/api/pd/notifications", {
