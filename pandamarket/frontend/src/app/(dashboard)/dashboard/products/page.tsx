...
export default function ProductsPage() {
  ...
  // 2. Hoist search.toLowerCase() outside the filter loop
  // 3. Early return if search is empty
      ...
      >
        <div>
          ...
          >
            {isLoading
              ? "Chargement..."
              : `${products.length} / ${MOCK_MAX_PRODUCTS} produits (Plan Actuel)`}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Produit", "Prix", "Stock", "Images", "Statut", ""].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
              ...
              <tbody>
                {filtered.map((p: any) => (
                  ...
                  >
                    ...
                    >
                      {p.images?.length || (p.thumbnail ? 1 : 0)}
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                      <Badge
                        variant={
                          statusBadge[p.status || "draft"]?.variant || "neutral"
                        }
                        dot
                      >
