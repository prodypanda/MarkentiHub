...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, hint, icon, style, id, ...props }, ref) => {
    ...
    const labelStyle: React.CSSProperties = {
      fontSize: 'var(--pd-fs-sm)', fontWeight: 500, color: 'var(--pd-text-primary)',
    };
    ...
    const iconStyle: React.CSSProperties = {
      position: 'absolute', left: '12px', color: 'var(--pd-text-tertiary)',
      ...
      <div style={containerStyle}>
        ...
        <div style={wrapperStyle}>
          ...
          <input
            ...
            onBlur={(e) => {
              e.currentTarget.style.borderColor = hasError ? 'var(--pd-red)' : 'var(--pd-border)';
              e.currentTarget.style.boxShadow = 'none';
            }}
            {...props}
          />
        </div>
        {error && <span id={errorId} aria-live="polite" style={{ fontSize: 'var(--pd-fs-xs)', color: 'var(--pd-red)' }}>{error}</span>}
